// index.js - Discord Bot (Fixed Complete Version)
...
async function connectMongo() {
    try {
        ...
        console.error('❌ Bot: MongoDB connection failed:', error);
        process.exit(1);
    }
}
...
async function handleVerifyCommand(message, args) {
    ...
    await VerificationSystem.verifyUser(message, username, targetMember);
}
...
async function handleWarnCommand(message, args) {
    ...
    const warningCount = await WarningSystem.addWarning(
        message.guild.id,
        targetMember.id,
        message.author.id,
        reason
    ...
    const config = await Utils.getConfig(message.guild.id, 'warnings');
    const actionTaken = await WarningSystem.executeWarningAction(message.guild, targetMember, warningCount, config);

    let response = `⚠️ **${targetMember.displayName}** has been warned.\n**Reason:** ${reason}\n**Warning Count:** ${warningCount}`;
    
    if (actionTaken) {
        response += `\n🔨 **Action taken:** ${config.warningAction || 'kick'} (reached ${config.maxWarnings || 3} warnings)`;
    }
...
async function handleLeaderboardCommand(message) {
    ...
    for (let i = 0; i < leaderboard.length; i++) {
        ...
        description += `${medal} **${member?.displayName || 'Unknown User'}** - Level ${level} (${user.xp} XP)\n`;
    }
...
async function handleAvatarCommand(message, args) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor('#0099ff')
        .setFooter({ 
            text: `PNG: ${targetUser.displayAvatarURL({ format: 'png', size: 512 })}\nWebP: ${targetUser.displayAvatarURL({ format: 'webp', size: 512 })}${targetUser.avatar && targetUser.avatar.startsWith('a_') ? `\nGIF: ${targetUser.displayAvatarURL({ format: 'gif', size: 512 })}` : ''}` 
        });

    message.reply({ embeds: [embed] });
}

async function handlePinMessageCommand(message, args) {
    if (!message.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
        return message.reply('❌ You need manage messages permission to use this command.');
    }

    if (args.length === 0) {
        return message.reply('❌ Usage: `!pinmessage <config_name>`');
    }

    const configName = args[0];
    const config = await Utils.getConfig(message.guild.id, `pinned_${configName}`);
    
    if (!config.title && !config.description) {
        return message.reply('❌ Pin message configuration not found. Use `!config` to set it up.');
    }

    const embed = Utils.createEmbed(config);
    const sentMessage = await message.channel.send({ embeds: [embed] });
    await sentMessage.pin();
    
    message.reply(`✅ Pinned message sent and pinned!`);
}

async function handleSetChannelCommand(message, args) {
    if (!message.member.permissions.has(PermissionFlagsBits.ManageChannels)) {
        return message.reply('❌ You need manage channels permission to use this command.');
    }

    if (args.length < 2) {
        return message.reply('❌ Usage: `!setchannel <channelId> <commandOnly|mediaOnly|chatOnly|verifyChannel|normal>`');
    }

    const channelId = args[0].replace(/[<#>]/g, '');
    const type = args[1].toLowerCase();
    
    const validTypes = ['commandonly', 'mediaonly', 'chatonly', 'verifychannel', 'normal'];
    if (!validTypes.includes(type)) {
        return message.reply('❌ Invalid channel type. Valid types: commandOnly, mediaOnly, chatOnly, verifyChannel, normal');
    }

    await ChannelPermissions.setChannelType(message.guild.id, channelId, type);
    message.reply(`✅ Channel <#${channelId}> set to **${type}** mode.`);
}

async function handleSendEmbedCommand(message, args) {
    if (!message.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
        return message.reply('❌ You need manage messages permission to use this command.');
    }

    if (args.length === 0) {
        return message.reply('❌ Usage: `!sendembed <config_name>`');
    }

    const configName = args[0];
    const config = await Utils.getConfig(message.guild.id, `embed_${configName}`);
    
    if (!config.title && !config.description) {
        return message.reply('❌ Embed configuration not found. Use `!config` to set it up.');
    }
...
async function handleConfigCommand(message, args) {
    ...
    if (args.length === 0) {
        return message.reply(`❌ Usage: \`!config <type>\`
**Available types:**
• \`verification\` - Set up Roblox verification
• \`welcome\` - Configure welcome messages  
• \`goodbye\` - Configure goodbye messages
• \`warnings\` - Set up warning system
• \`leveling\` - Configure XP system
• \`embed_<n>\` - Create custom embed
• \`pinned_<n>\` - Create pinned message config`);
    }

    const type = args[0];
    message.reply(`⚙️ To configure **${type}**, you need to update the MongoDB config collection directly. Here's the structure:
...
Please refer to the documentation for specific configuration options.`);
}
